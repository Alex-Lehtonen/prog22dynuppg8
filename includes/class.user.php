...
class User {
 ...
 private function cleanInput($data){
	...
	public function checkUserRegisterInput($uname, $umail, $upass, $upassrepeat){
	  ...
	  $stmt_checkUsername = $this->pdo->prepare('SELECT * FROM user WHERE u_name = :uname OR u_email = :email');
 ...
 public function register($uname, $umail, $upass){
	...
	$uname = $this->cleanInput($uname);
	 ...
	 $stmt_registerUser = $this->pdo->prepare('INSERT INTO user (u_name, u_password, u_email, u_role_fk, u_status) VALUES (:name, :pw, :email, 1, 1)');
	...
	$stmt_checkUsername = $this->pdo->prepare('SELECT * FROM user WHERE u_name = :uname OR u_email = :email');
	  ...
	  $stmt_checkUsername->execute();
	   ...
	   if(password_verify( $upass, $userData['u_password'])){
		...
		array_push($this->errorMessages, "wrong password");
		return $this->errorMessages;
	}
...
public function checkUserRole($val){
	$stmt_checkUserRoleLevel = $this->pdo->prepare('
	SELECT * FROM roles WHERE r_id = :rid
	');
		$stmt_checkUserRoleLevel->bindParam(":rid", $_SESSION['user_role'], PDO::PARAM_STR);
